...
var ImageUpload = function (_Component) {
	...
	function ImageUpload(props) {
		...
		_this.callbackUploader = function (file, files, fileIndex) {
			_this.beforeUploading(files, fileIndex);
			_this.props.uploadImageCallback(file).then(function (res) {
				_this.afterUploading();
				if (res && res.status == "success") {
					_this.updateImage(res.data['image_src']);
				}
			}).catch(function (error) {
				console.log('error', error);
				_this.errorUploading();
			});
		};
